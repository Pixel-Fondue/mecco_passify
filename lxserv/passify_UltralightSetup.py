...
class cmd_setup_class(lxu.command.BasicCommand):
    ...
    def CMD_EXE(self, msg, flags):
        include_environments = self.dyna_Bool(0) if self.dyna_IsSet(0) else True
        include_lumigons = self.dyna_Bool(1) if self.dyna_IsSet(1) else True
        headroom = self.dyna_Float(2) if self.dyna_IsSet(2) else 0.0

        passify.ultralight.build(include_environments, include_lumigons, headroom)
