...
def list_passes():
    ...
    passes_list = []
    for p in passes:
        passes_list.append(passify.CMD_TOGGLER_ACTIVATE + " {%s}" % p.id)

    passes_list.append(passify.CMD_TOGGLER_ACTIVATE + " {%s}" % passify.NONE)
...
lx.bless(cmd_passify_fcl, passify.CMD_TOGGLER_FCL)
